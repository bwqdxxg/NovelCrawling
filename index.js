...
const { integration, masterStationClassBooksMethods, allBooksSaveLocalhost, bookContent, crawlNovel, everyClassBooks, classBooks } = require('./crawl').api
 ...
 * 4.用户打开书籍介绍或目录时，对应的获取接口自动去指定源的书籍目录与服务器的进行对比；相同则用本地的，若不同则重新爬取新的目录
 * 5.每周或半月重新爬取一次，将本地没有的爬取下来
 * 页面组成部分：
 * 1.个人书架/书城/我的（暂定）
 * 2.点击书籍=》书籍介绍（书籍封面/介绍/加入书架/其他书籍推荐/全本缓存/阅读/目录）
 * 3.【书籍封面/介绍/加入书架/其他书籍推荐】使用自己的；【全本缓存/阅读/目录】使用对应源头爬取的
 * 未完成：
 * 1.爬取源无效/无法获取，自动跳到下个源；爬取书籍过程超时自动重连，重连次数超过指定数自动跳过开始爬取下一本
 * 2.优化load模块
 * 3.检查书籍目录是否更新
 */
